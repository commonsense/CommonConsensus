...
{Geoffrey Grosenbach}[mailto:boss@topfunky.com] -- http://topfunky.com -- http://newbieonrails.topfunky.com 
...
To use, require 'spark' and call it thusly:

	Spark.plot([1,25,33,46,89,90,85,77,42], :type => 'discrete', :height => 20)

An image blob will be returned which you can print, write to STDOUT, etc.

In Rails, copy the spark_controller and spark_helper files to the appropriate folders. Copy spark.rb to your 'lib' folder.

In your custom controller, call

  require_dependency 'spark'
...
<%= spark_tag [1,2,3,4,5,6] %> <!-- Gives you a smooth graph -->

Or specify details:

<%= spark_tag [1,2,3,4,5,6], :type => 'discrete', :height => 10, :upper => 80, :above_color => 'green', :below_color => 'blue' %>
...
module Spark
	...
	# Writes a graph to disk with the specified filename, or "spark.png"
	def Spark.plot_to_file(filename="spark.png", results=[], options={})
		File.open( filename, 'wb' ) do |png|
	 		png << Spark.plot( results, options)
		end
	end
