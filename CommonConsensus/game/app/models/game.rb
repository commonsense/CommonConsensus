...
class Game
	...
	def setParameters
	   ...
	   @goal.goal_used += 1
	   @goal.save!
	   goalcount = self.find_by_sql("select count(*) as gc from goals")
	   gc = goalcount[0].gc
	   sparsity = self.find_by_sql("(select count(*) as sp from guesses where game_type = 0 and goal_id ="+@goal.id.to_s+") union (select count(*) as sp from guesses where game_type = 1 and goal_id = "+@goal.id.to_s+") union (select count(*) as sp from guesses where game_type = 2 and goal_id ="+@goal.id.to_s+") union (select count(*) as sp from guesses where game_type = 3 and goal_id = "+@goal.id.to_s+")")
	   averages = self.find_by_sql("(select count(*)/"+gc.to_s+" as av from guesses where game_type = 0) union (select count(*)/"+gc.to_s+" as av from guesses where game_type = 1) union (select count(*)/"+gc.to_s+" as av from guesses where game_type = 2) union (select count(*)/"+gc.to_s+" as av from guesses where game_type = 3)")
