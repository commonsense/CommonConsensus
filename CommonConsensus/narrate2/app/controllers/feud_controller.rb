...
def submit_feedback
	...
	if @params[:commit] != "No Thanks!"
	   ...
	   Feedback.create(:first => @quest[:first], :second => @quest[:second], :third => @quest[:third], :fourth => @quest[:fourth], :fifth => @quest[:fifth], :sixth => @quest[:sixth], :userid => @session[:user].id)
