        ...
        # first look for identicial matches
	  	@guesses.each do |g|
	  	   if g.contains?(parsedguess)
	  	  	  ...
	  	  	  gamecat = g.catid
	  	  	  found = true
	  	  	  puts "FOUND IN CATEGORY #{parsedguess}  #{gamecat}"
	  	  	  break
	  	   end
        end 
        ...
        if not found
           @guesses.each do |g|
              ...
              if similarity >= 0.75
                  ...
                  found = true
                  break
                  puts "FOUND IN GLUENET SIM: #{similarity}  #{gamecat}"
              end
	  	   end
        end
	  	if not found
	   	...
	   	# add guess to db
	    g = add_guess_to_db(in_g,@goal.id,gamecat,1,@gametype,@players.size())
	    return gamecat
	end
...
def gradeAnswers(contents)
	 	...
	 	return []
            ...
            @guesses.sort!.reverse!
                ...
                for gs in @guesses
                        ...
                        x = Guess.find(:first, :conditions => ["g_both = ? and  game_type = ? and goal_id = ?", gs.string, @gametype, @goal.id])
                        if x
                          x.players += @players.size()
                          x.save!
                        else
                          puts "ERROR: Goal undefined", gs.string,@gametype,@goal.id
 ...
 def add_guess_to_db(in_g, goal, categ, times_chosen=1, game_type=1, players=0)
  ...
  if not x
  	...
  	ruletypes[type], :num => times_chosen, 
  		:g_verb => body, :g_noun => subject, :g_both => both, :goal_id => goal, :category => categ, :players => players)
  else 
    x.num += 1
    x.save!
  end
